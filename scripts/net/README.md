...
| `AuthService.gd` | 设备令牌登录；邮箱绑定/找回；**上传头像**；暴露 `is_online` / `player_id` / `email` / `avatar_url` |
...
16. **自定义头像（圆形）**：账号面板（N）"上传头像"选图→服务端压缩存储。头像统一**裁剪为圆形**
    （`AvatarUtil.circle_texture`，中心裁方+圆形遮罩），并显示在：账号面板预览、**聊天消息**（昵称前）、
    **群聊成员列表**、**排行榜行**、**世界地图同伴**（实时同伴头顶 + 异步幽灵头顶的圆形头像牌）。
    历史/榜单/同伴的头像按 `player_id` 经 `GET /players/avatars` 批量拉取并缓存。

15. **群聊/临时房间 · 引用回复 · 自定义贴图包**：频道选 **群聊**，在房间码栏填名字点"建"创建**带名群聊**，
    或填码点"进"加入（跨章节、含历史）；"成员"按钮看**房间名 + 在线成员列表**（实时更新）。任意消息点 **引**
    可**引用回复**，引用块**可点击跳转**到原消息并高亮；**贴图包支持分组**（组下拉/新建组/＋添加到当前组/右键删除，本地持久化）。
