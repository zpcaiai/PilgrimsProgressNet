...
func _build_chapter() -> void:
	...
	make_floating_label("战斗测试场 Combat Test Arena", Vector3(0, 4, -18), Color(1, 0.9, 0.8))
	...
	var _cb1 := func(_p):
		...
		EventBus.toast("你领受一条应许：应许次数 +1。")
	make_interactable(Vector3(-8, 0, 2), "领受应许 (Promise +1)",
		_cb1, null, Color(0.9, 0.85, 0.5), 1.5, 1.2, false)

	# Summon the boss.
	make_interactable(Vector3(8, 0, 2), "召出亚玻伦 (Summon Apollyon)",
...
func _start_boss() -> void:
	...
	_boss_controller.fight_ended.connect(func(_v): EventBus.toast("山谷安静下来：控告已经破碎。"))
