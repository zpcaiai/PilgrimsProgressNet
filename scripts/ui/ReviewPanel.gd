...
const FONT_BODY := 17
const BOARD_NAMES := {
	"fastest_run": "最快通关", "fewest_falls": "最少倒下", "devout_score": "敬虔之心",
}
const STATUS_NAMES := {
	"pending": "待重算", "approved": "已通过", "rejected": "已拒绝", "appealed": "申诉中",
}
const STATUS_COLORS := {
...
func _refresh() -> void:
	...
	if not NetConfig.enabled or not AuthService.is_online:
		_status.text = "离线模式 — 联网后可查看成绩复核状态。"
		return
	_status.text = "加载中…"
...
func _on_reviews(reviews: Array) -> void:
	...
	for c in _list.get_children():
		c.queue_free()
	if reviews.is_empty():
		_status.text = "你没有被标记的成绩，一切清白。"
		return
	_status.text = "共 %d 条被标记的成绩：" % reviews.size()
	for r in reviews:
		_add_row(r)
...
func _add_row(r: Dictionary) -> void:
	...
	info.text = "[b]%s[/b]　分数 %d　[color=#%s]%s[/color]\n[color=#8890a0]%s[/color]" % [
	...
	if st == "rejected":
		...
		btn.pressed.connect(func(): LeaderboardService.appeal_review(rid, "请求人工复核"))
		hb.add_child(btn)
